...
def prepare_hospital_data(base_data_path=None, output_dir=None, splits=[150, 175]):
    ...
    for i, num_samples in enumerate(splits):
        ...
        # Copy normal images
        copied_normal = 0
        while copied_normal < num_normal_for_hospital and current_normal_idx < len(normal_images):
        ...
        copied_pneumonia = 0
        while copied_pneumonia < num_pneumonia_for_hospital and current_pneumonia_idx < len(pneumonia_images):
            shutil.copy(pneumonia_images[current_pneumonia_idx], hospital_path / "PNEUMONIA")
            current_pneumonia_idx += 1
            copied_pneumonia += 1
        
        print(f"Finished {hospital_id}: Copied {copied_normal} NORMAL, {copied_pneumonia} PNEUMONIA.")

    if current_normal_idx < len(normal_images) or current_pneumonia_idx < len(pneumonia_images):
